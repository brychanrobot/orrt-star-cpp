...
void generateObstacleRects(int width, int height, int depth, int count, std::vector<Rect*>& obstacles) {
	for (int x = 0; x < count; x++) {
		Rect* rect;
		while (true) {
			auto topLeft = randomPoint(width, height, depth);
			auto bottomRight = randomPoint(width, height, depth);
