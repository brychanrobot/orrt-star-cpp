...
Rect::Rect(Coord tl, Coord br) {
	...
	this->topLeft = Coord(minx, miny, 0);
	this->bottomRight = Coord(maxx, maxy, 0);
}

void Rect::inflate(double dx, double dy) {
	this->topLeft = Coord(this->topLeft.x() - dx, this->topLeft.y() - dy, 0);

	this->bottomRight = Coord(this->bottomRight.x() + dx, this->bottomRight.y() + dy, 0);
}
